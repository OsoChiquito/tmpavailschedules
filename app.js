    ...
    if (S.showWindows) {
      windowsFor(key).forEach(function (w) {
        ...
        box.innerHTML = '<span class="t">' + hhmm(w.s) + '-' + hhmm(w.e) + '</span>' +
                        (mins >= 45 ? '<span class="n">' + esc(label) + '</span>' : '');
        box.title = hhmm(w.s) + '-' + hhmm(w.e) + ', ' + dur(mins) + ', ' + label;
...
function init() {
  ...
  ['toggleWindows|showWindows', 'toggleOthers|showOthers', 'toggleAttendees|showAttendees'].forEach(function (pair) {
    var p = pair.split('|');
