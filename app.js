...
function windowsFor(dateIso) {
  if (!picked.length || !loadedDates[dateIso]) return [];
...
function visibleEvents(dateIso) {
  ...
  if (!picked.length || S.showOthers) return all.map(tag);
...
function drawBoard() {
  ...
  for (var d = 0; d < 5; d++) {
    ...
    if (S.showWindows) {
      windowsFor(key).forEach(function (w) {
        ...
        var label = full ? (picked.length > 1 ? 'all ' + picked.length + ' free' : 'free')
    ...
    stack(visibleEvents(key)).forEach(function (ev) {
      ...
      box.style.height = bh + 'px';
      /* Events start after the 64px availability strip and share what is left. */
      var frac = 1 / ev.cols;
      box.style.left = 'calc(70px + (100% - 74px) * ' + (ev.col * frac) + ')';
      box.style.width = 'calc((100% - 74px) * ' + frac + ' - 4px)';
  ...
  if (S.showNow) {
    ...
    for (var k = 0; k < 5; k++) {
      if (iso(addDays(weekStart, k)) !== nowIso) continue;
...
function buildShareText(mode) {
  var lines = [];
  var who = picked.length ? joinNames(picked) : 'everyone on the schedule';
  var a = weekStart, b = addDays(weekStart, 4);
  ...
  if (!picked.length) { lines.push('Pick at least one name on the board first.'); return lines.join('\n'); }
...
function drawPrint() {
  var a = weekStart, b = addDays(weekStart, 4);
  var who = picked.length ? joinNames(picked) : 'the whole schedule';
  $('printOut').innerHTML =
...
function fitDay() {
  ...
  for (var i = 0; i < 5; i++) {
    eventsOn(iso(addDays(weekStart, i))).forEach(function (ev) {
      if (ev.allDay) return;
      if (picked.length && !ev.people.some(function (p) { return picked.indexOf(p) >= 0; })) return;
...
function init() {
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'ArrowLeft') { weekStart = addDays(weekStart, -7); drawAll(); }
    else if (e.key === 'ArrowRight') { weekStart = addDays(weekStart, 7); drawAll(); }
    else if (e.key === 't' || e.key === 'T') { weekStart = mondayOf(new Date()); drawAll(); }
  ...
  ['toggleWindows|showWindows', 'toggleOthers|showOthers',
   'toggleAttendees|showAttendees', 'toggleNow|showNow'].forEach(function (pair) {
    var p = pair.split('|');
